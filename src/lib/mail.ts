...
export function orderStatusEmail(orderNo: string, status: string, link: string) {
  const messages: Record<string, { subject: string; text: string }> = {
    paid: { subject: '订单已付款', text: `您的订单 ${orderNo} 已付款成功，卖家将尽快发货。` },
    shipped: { subject: '订单已发货', text: `您的订单 ${orderNo} 已发货，请注意查收快递。` },
    delivered: { subject: '订单已确认收货', text: `订单 ${orderNo} 已确认收货。` },
    completed: { subject: '订单已完成', text: `订单 ${orderNo} 已完成，感谢使用闲妙。` },
    cancelled: { subject: '订单已取消', text: `订单 ${orderNo} 已取消。` },
    refunding: { subject: '退款申请已提交', text: `订单 ${orderNo} 的退款申请已提交，等待卖家处理。` },
  }
  const msg = messages[status] || { subject: '订单状态更新', text: `订单 ${orderNo} 状态已更新。` }
  return {
    ...
    html: `
      <div style="font-family:system-ui,sans-serif;max-width:480px;margin:0 auto;padding:24px">
        ...
        <a href="${link}" style="display:inline-block;margin-top:16px;padding:10px 24px;background:#111;color:#fff;border-radius:8px;text-decoration:none;font-size:14px">查看订单</a>
      </div>
    `,
  }
}
    ...
    return {
      ...
      html: `
        <div style="font-family:system-ui,sans-serif;max-width:480px;margin:0 auto;padding:24px">
          ...
          <p style="color:#555;font-size:14px;line-height:1.6">您的店铺「${shopName}」已审核通过，现在可以开始上架商品了。</p>
  ...
  return {
    ...
    html: `
      <div style="font-family:system-ui,sans-serif;max-width:480px;margin:0 auto;padding:24px">
        ...
        <p style="color:#555;font-size:14px;line-height:1.6">您的店铺「${shopName}」入驻申请未通过审核。${reason ? `原因：${reason}` : ''}</p>
